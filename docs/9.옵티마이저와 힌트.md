...
- Extra 칼럼에 이런 말들이 포함되지 않는다. `Using index for Group By` or `Using temporary, Using filesort`
...
GROUP BY emp_no
```
- (emp_no, from_date) 로 인덱스가 걸려있다고 가정해보자. 
- 인덱스를 사용할 수 없겠다고 느낄 수 있는데 인덱스를 사용한다. 
- 인덱스에서 emp_no 를 처음 읽고, 그 값 중에서 from_date 가 1985-03-01 만 뽑아온다. 그런식으로 진행함.
...
- ORDER BY 또는 GROUP BY 칼럼이 조인의 첫번째 테이블이 아닌 경우 
